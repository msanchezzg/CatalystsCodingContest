...
def main(argv):
    ...
    for task_id, task_power, task_st, task_end in tasks:
        ...
        while task_power > 0:
            ...
            if left > 0 and concurrent_tasks.get(cheap_min, 0)+1 <= max_tasks:
                if left == 1 or power_ini == 1 or task_power == 1:
                    power_drained = 1
                else:
                    power_drained = random.randint(2, min(left, task_power))
                ...
                if cheap_min in concurrent_tasks:
                    concurrent_tasks[cheap_min] += 1
                else:
                    concurrent_tasks[cheap_min] = 1

            # else:
                # print(f'{cheap_min} not available')
    ...
    #Check_bill
    bill = 0
    for m in price_per_min:
        bill += (price_per_min[m]*mins_tasks.get(m, 0))
    if bill > max_bill:
        print('ERROR')
